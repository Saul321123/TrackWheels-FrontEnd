...
function DeviceAssignmentList() {
  ...
  useEffect(() => {
    const fetchAssignments = async () => {
      try {
        const response = await fetch('http://44.207.165.192:3000/asignaciones/listado');
        if (!response.ok) {
  ...
  const handleUnassign = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`http://44.207.165.192:3000/asignaciones/desasignar/${id}`, {
          method: 'DELETE',
        });
